...
import {AddSubmissionFormRequest, AddSubmissionFormSchema} from "@/lib/form.service";
...
const formSubmissionExclusion: {
  field: 'exclusion_bibliographic' | 'exclusion_quoted' | 'exclusion_small_sources',
  label: string
} [] = [
  {
...
export const AddSubmissionModal = ({open, setOpen, callback}: AddSubmissionModalProps) => {
  ...
  const isExclusionSmallSources = useWatch({control: formMethods.control, name: 'exclusion_small_sources'})
  const exclusionType = useWatch({control: formMethods.control, name: 'exclusion_type'})
  const exclusionValue = useWatch({control: formMethods.control, name: 'exclusion_value'})
  ...
  const onSubmit = (data: AddSubmissionFormRequest) => {
    ...
    formData.append('exclusion_small_sources', String(data.exclusion_small_sources));
    formData.append('exclusion_bibliographic', String(data.exclusion_bibliographic));
                  ...
                  <Fieldset.Root>
                    ...
                    <Fieldset.Content display={'flex'} flexDirection={{base: 'column', md: 'row'}} gap={6}>
                      ...
                      <Flex flexDirection={'column'} gap={4} flex={'1'} position={'relative'}>
                        ...
                        <Field.Root invalid={!!formMethods.formState.errors.title}>
                          <Field.Label>Title</Field.Label>
                          <Input {...formMethods.register('title')} name="title" type="text"
                                 placeholder={'Enter the document title'}/>
                          <Field.ErrorText>{formMethods.formState.errors.title?.message}</Field.ErrorText>
                        </Field.Root>
                        ...
                        <Box mb={3}>
                          ...
                          <Stack gap={4} align="flex-start">
                            <For each={formSubmissionExclusion}>
                              {(item) => (
                                <SwitchComponent key={item.field} name={item.field} label={item.label}/>
                              )}
                            </For>
                          </Stack>
                        </Box>
                        ...
                        >
                          <Flex justifyContent={'space-between'}>
                            ...
                            <RadioGroup.Root defaultValue="1" size={'sm'} variant={'outline'}
                                             ...
                                             onValueChange={(details) => {
                                               // @ts-ignore
                                               formMethods.setValue('exclusion_type', details.value)
                                             }}
                            >
                              <HStack gap={3}>
                          ...
                          <Flex>
                            <NumberInput.Root
                              ...
                              onValueChange={({value}) => {
                                formMethods.setValue('exclusion_value', Number(value))
                              }}
                            >
                              <NumberInput.Label/>
                              <NumberInput.Control>
                      ...
                      >
                        ...
                        <FileUpload.Dropzone minHeight={'auto'}
                          ...
                          <FileUpload.DropzoneContent>
                            <Box fontSize={'sm'} mb={5}>Drag and drop file here</Box>
                        ...
                        <Text textStyle={'xs'} color={'red.500'}
                              fontWeight={500}>{formMethods.formState.errors.file?.message as string}</Text>
                      </FileUpload.Root>
                    </Fieldset.Content>
...
const SwitchComponent = ({label, name}: {
  label: string,
  name: 'exclusion_bibliographic' | 'exclusion_quoted' | 'exclusion_small_sources'
}) => {
  ...
  return (
    <Switch.Root size={'sm'} variant={'solid'} cursor={'pointer'}
                 checked={fieldValue}
