...
import {Button, CloseButton, Dialog, Field, Fieldset, Input, InputGroup, Portal, Stack, Switch} from "@chakra-ui/react";
...
export const AddUserModal = ({selectedUser, open, setOpen, callback}: AddUserModalProps) => {
            ...
            <FormProvider {...formMethods}>
              <form onSubmit={formMethods.handleSubmit(onSubmit)}>
                  ...
                  <Fieldset.Root>
                    ...
                    <Fieldset.Content>
                      ...
                      <Field.Root invalid={!!formMethods.formState.errors.name} flex={1}>
                        ...
                        <Input {...formMethods.register('name')} name="name" type="text"
                      ...
                      <Field.Root invalid={!!formMethods.formState.errors.email} flex={1}>
                        ...
                        <Input {...formMethods.register('email')} name="email" type="email"
                               placeholder={'Enter the student email'}/>
                        <Field.ErrorText>{formMethods.formState.errors.email?.message}</Field.ErrorText>
                      </Field.Root>

                      {!selectedUser && (
                        <Field.Root invalid={!!formMethods.formState.errors.password} flex={1}>
                          <Field.Label>Password</Field.Label>
                          <InputGroup flex="1"
                                      endElement={<Button size={'2xs'} onClick={() => {
                                        formMethods.setValue('password', generatePassword())
                                      }}>Generate <IoMdRefresh/></Button>}>
                            <Input  {...formMethods.register('password')} name="password" type="text"
                                    placeholder={'Enter the student password'}/>
                          </InputGroup>
                          <Field.ErrorText>{formMethods.formState.errors.password?.message}</Field.ErrorText>
                        </Field.Root>
                      )}
                ...
                <Dialog.Footer>
                  ...
                  <Button loading={formMethods.formState.isSubmitting}
                          type={'submit'}>{!selectedUser ? 'Submit' : 'Update'} <MdArrowRightAlt/></Button>
                </Dialog.Footer>
