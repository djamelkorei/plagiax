...
export default function TemplateEmail({title, children}: { title: string, children?: ReactNode }) {
  return (
    <Html>
      <Head>
      ...
      <Tailwind config={tailwindConfig}>
        <Body className="bg-[#fafafa] font-geist">
          ...
          <h1 className={'font-geist-mono font-bold text-2xl tracking-wide text-center py-5'}>
          ...
          <Container className="p-10 pt-5 mx-auto bg-[#FFF]">
            ...
            <Text className="text-[#777] text-[14px] leading-[1.5] m-0!">
              This email was sent via <a className={'text-[#555]'} href={AppContext.url}>{AppContext.name}</a>.
            </Text>

            <Text className="text-[#777] text-[14px] leading-[1.5] m-0!">
              If you require any assistance, don’t hesitate to reach out at : <a
              className={'text-[#555] text-underline'}
              href={'mailto:' + AppContext.email}>{AppContext.email}</a>
            </Text>
          ...
          <Container className="px-10 py-5 text-center">
            <Text className="text-[#777] text-[12px] leading-[1.5] m-0!">
              @ {new Date().getFullYear()} {AppContext.name} | {AppContext.domain}
            </Text>
          </Container>
        </Body>
      </Tailwind>
