...
export async function submissionAddAction(formData: FormData): Promise<{ hasError: boolean, message: string }> {
  ...
  if (settings?.submission_action === "MANUAL") {
    ...
    if (pendingCount >= 1) {
      return {
        hasError: true,
        message: "The maximum number of pending submissions is 1. Please wait until the pending submissions are processed."
      };
    }
  }
