...
export async function accountPasswordUpdate(formData: FormData): Promise<{ hasError: boolean, message: string }> {
  ...
  const match = await bcrypt.compare(parsed.data.current_password, current!.password);
  if (!match) return {hasError: true, message: "Password does match your current password"};
