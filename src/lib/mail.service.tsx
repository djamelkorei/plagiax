...
const sendWelcomeEmail = async (userName: string, userEmail: string, link: string, password: string) => {
  const subject = `Welcome to ${AppContext.name}!`;
  const template = <WelcomeEmailTemplate userName={userName} userEmail={userEmail} userPassword={password}
                                         link={link}/>;
